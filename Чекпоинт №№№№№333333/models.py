...
class User(UserMixin, db.Model):
    ...
    specialization = db.Column(db.String(100), nullable=True)  # 'Психолог', 'Юрист', 'Конфликтолог', 'Соцпедагог'
...
class Appeal(db.Model):
    ...
    rating = db.Column(db.Integer, nullable=True)                 # Обязательная оценка 1-5
    ...
    co_expert_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)  # Соисполнитель
    ...
    co_expert = db.relationship('User', foreign_keys=[co_expert_id])
